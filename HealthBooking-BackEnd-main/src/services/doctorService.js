    ...
    try {
      let doctors = await db.Booking.findAll({
        attributes: [
          ...
          [
            Sequelize.fn("COUNT", Sequelize.col("Booking.doctorId")),
            "appointmentCount",
          ],
        ],
        include: [
        ...
        group: ["doctorId", "User->positionData.id", "User->Doctor_Infor.id"],
        order: [
...
// ////////////////////
const getTimeBookingByDate = (doctorId, date) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!doctorId || !date) {
        resolve({
