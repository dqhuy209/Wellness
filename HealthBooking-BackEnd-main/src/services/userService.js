    ...
    try {
      ...
      if (isExist) {
        ...
        let user = await db.User.findOne({
          attributes: ["email", "passWord", "roleId", "firstName", "lastName"], // Chỉ lấy ra các cột này
          where: {
            email: email,
          },
          raw: true,
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (checkEmail == true) {
        ...
        let hashPassWordFromByScript = await hashUserPassWord(data.passWord); //lấy passWord đc hash promise trả về
    ...
    try {
      if (
        ...
        if (checkEmail == true) {
          ...
          let hashPassWordFromByScript = await hashUserPassWord(data.passWord);
...
// ////////////////////////////
let handleUserLogin3 = (username, password) => {
  return new Promise(async (resolve, reject) => {
    try {
      let userData = {};
      let isExist = await checkUserEmail(username);
