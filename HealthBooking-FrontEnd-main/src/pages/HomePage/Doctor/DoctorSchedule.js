  ...
  getArrDays = (language) => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (language === LANGUAGE.VI) {
        if (i === 0) {
          const ddMM = moment(new Date()).add(i, "days").format("DD/MM");
...
//       isOpenModalBooking: false,
//       dataScheduleTimeModal: {},
//       bookedTimes: {}, // Object để lưu trữ trạng thái đặt của từng thời gian
//     };
//   }
...
//   //   const updatedBookedTimes = { ...bookedTimes, [time.id]: true }; // Đánh dấu thời gian đã đặt thành true
//   //   this.setState({ bookedTimes: updatedBookedTimes });
//   // };

//   updateBookingStatus = (timeId, isBooked) => {
//     const { bookedTimes } = this.state;
//     const updatedBookedTimes = { ...bookedTimes, [timeId]: isBooked };
//     this.setState({ bookedTimes: updatedBookedTimes });
//   };
...
//                   <div className="times-content">
//                     {allAvalableTime.map((item, index) => {
//                       const isBooked = this.state.bookedTimes[item.id]; // Sử dụng ID của thời gian làm key
//                       return (
//                         <button
//                           key={index}
//                           className={`${
//                             language === LANGUAGE.VI ? "btn-vie" : "btn-en"
//                           } ${
//                             this.state.bookedTimes[item.id] ? "btn-booked" : ""
//                           }`}
//                           onClick={() => {
//                             if (!isBooked) {
//                               this.handleClickScheduleTime(item);
